        ...
        VolleyRequestUtils.getInstance().getCookieRequest(this, url, new VolleyRequestUtils.HttpStringRequsetCallBack() {
            ...
            public void onSuccess(String result) {
                ...
                if (error == 0) {
                    ...
                    /*是否自带主石*/
                    //isSelfStone = modelEntity.getIsSelfStone();
...
//        viewPager.setLayoutParams(linearParams); // 使设置好的布局参数应用到控件myGrid
    ...
    private void chkeckConfirmOrder() {
        ...
        if (isNumber(storeNumber)) {
            /*自带主石头*/
            if (isSelfStone == 1) {
                L.e("checkStone"+checkStone());
                if (checkStone()) {
                    isConfir = true;
                }
            } else {
                L.e("noMaincheckStone"+noMaincheckStone());
                if (noMaincheckStone()) {
                    isConfir = true;
                }
            }
    ...
    /*不自带主石头验证*/
    private boolean noMaincheckStone() {
        if (checkUpPass(stone) && checkStone()) {
            return false;
        }
        return true;
    }
    ...
    private boolean checkStone() {
        L.e("自带主石头" + stone.toString());
        if (checkUpPass(stoneA) && checkUpPass(stoneB) && checkUpPass(stoneC)) {
            return false;
        }
        return true;
    }
    ...
    private void addCurentOrder() {
        ...
        if (type == 1) {
            ...
            url = AppURL.URL_CURRENT_EDIT_ORDER + "tokenKey=" + BaseApplication.getToken() + "&itemId=" + itemId + "&number=" + storeNumber
                    + "&handSize=" + storeSize + urlStroe + urlStroeA + urlStroeB + urlStroeC + "&isSelfStone=" + isSelfStone;
        } else if (type == 2) {
            ...
            url = AppURL.URL_CURRENT_ORDER + "tokenKey=" + BaseApplication.getToken() + "&productId=" + itemId + "&categoryId=" + categoryId +
                    "&number=" + storeNumber + "&handSize=" + storeSize + urlStroe + urlStroeA + urlStroeB + urlStroeC + "&isSelfStone=" + isSelfStone;
        }
        L.e("提交订单" + url);
        VolleyRequestUtils.getInstance().getCookieRequest(this, url, new VolleyRequestUtils.HttpStringRequsetCallBack() {
            @Override
    ...
    public boolean checkUpPass(StoneEntity stoEntity) {
        ...
        if (StringUtils.isEmpty(stoEntity.getNumber()) &&
                ...
                && StringUtils.isEmpty(stoEntity.getShapeId())&&StringUtils.isEmpty(stoEntity.getTypeTitle())&&
                StringUtils.isEmpty(stoEntity.getShapeTitle())) {
            isUpPass= false;
        }
        if (!StringUtils.isEmpty(stoEntity.getNumber()) &&
                !StringUtils.isEmpty(stoEntity.getPurityId()) && !StringUtils.isEmpty(stoEntity.getColorId())
                ...
                &&StringUtils.isEmpty(stoEntity.getTypeTitle())&&
                StringUtils.isEmpty(stoEntity.getShapeTitle())
                ) {
            isUpPass= false;
        }
    ...
    public String objectisEmptyAndtoJson(String key, StoneEntity stoEntity) {
        ...
        //如果是主石
        if (isSelfStone == 1 && key.equals("stone")) {
    ...
    int isSelfStone=0; /*是否是自带主石:0否，1是*/
    int isSelfStoneA=0; /*是否是自带主石:0否，1是*/
    int isSelfStoneB=0; /*是否是自带主石:0否，1是*/
    int isSelfStoneC=0; /*是否是自带主石:0否，1是*/
    ...
    public class ListAdapter extends BaseAdapter {
        ...
        public View getView(final int i, View view, ViewGroup viewGroup) {
            ...
            viewHolder.idIsCheck.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    if(checkedState.get(stoneEntity.getStroneName())!=null){
                        ...
                        if (!stoneEntity.getStroneName().equals("主 石")){
                            recoverStoreData(stoneEntity);
                        }
                    }else {
                        checkedState.put(stoneEntity.getStroneName(),getItem(i));
                    }
                    if (checkedState.size()>0){
                        for (String key:checkedState.keySet()){
                            ...
                            if (!stone.getStroneName().equals("主 石")){
                                cleanStoreData(stoneEntity);
                            }
            ...
            initDataAndListener(viewHolder,stoneEntity, i);
            return view;
        }
        ...
        /**历史数据**/
        List<StoneEntity> hisStoneEntity=new ArrayList<>();

        /***
         *
         * 如果反选封石  恢复数据
         * */
        public void recoverStoreData(StoneEntity mStoneEntity){
            for (int i=0;hisStoneEntity.size()>0&&i<hisStoneEntity.size();i++){
                if (hisStoneEntity.get(i).getStroneName().equals(mStoneEntity.getStroneName())){
                    L.e("恢复数据"+hisStoneEntity.get(i).toString());
                    mStoneEntity.setStroneName(hisStoneEntity.get(i).getStroneName());
                    mStoneEntity.setPrice(hisStoneEntity.get(i).getPrice());
                    mStoneEntity.setSpecId(hisStoneEntity.get(i).getSpecId());
                    mStoneEntity.setNumber(hisStoneEntity.get(i).getNumber());
                    mStoneEntity.setShapeId(hisStoneEntity.get(i).getShapeId());
                    mStoneEntity.setPurityTitle(hisStoneEntity.get(i).getPurityTitle());
                    mStoneEntity.setShapeTitle(hisStoneEntity.get(i).getShapeTitle());
                    mStoneEntity.setColorId(hisStoneEntity.get(i).getColorId());
                    mStoneEntity.setTypeId(hisStoneEntity.get(i).getTypeId());
                    mStoneEntity.setSpecTitle(hisStoneEntity.get(i).getSpecTitle());
                    mStoneEntity.setColorTitle(hisStoneEntity.get(i).getColorTitle());
                    mStoneEntity.setTypeTitle(hisStoneEntity.get(i).getTypeTitle());
                    mStoneEntity.setPurityId(hisStoneEntity.get(i).getPurityId());
                    mStoneEntity.setSpecSelectTitle(hisStoneEntity.get(i).getSpecSelectTitle());
                }
            }
        }
         ...
         * 如果勾选封石  就清空所有数据
         * */
        public void cleanStoreData(StoneEntity mStoneEntity) {
            StoneEntity stoneEntity = new StoneEntity();
            stoneEntity.setStroneName(mStoneEntity.getStroneName());
            stoneEntity.setPrice(mStoneEntity.getPrice());
            stoneEntity.setSpecId(mStoneEntity.getSpecId());
            stoneEntity.setNumber(mStoneEntity.getNumber());
            stoneEntity.setShapeId(mStoneEntity.getShapeId());
            stoneEntity.setPurityTitle(mStoneEntity.getPurityTitle());
            stoneEntity.setShapeTitle(mStoneEntity.getShapeTitle());
            stoneEntity.setColorId(mStoneEntity.getColorId());
            stoneEntity.setTypeId(mStoneEntity.getTypeId());
            stoneEntity.setSpecTitle(mStoneEntity.getSpecTitle());
            stoneEntity.setColorTitle(mStoneEntity.getColorTitle());
            stoneEntity.setTypeTitle(mStoneEntity.getTypeTitle());
            stoneEntity.setPurityId(mStoneEntity.getPurityId());
            stoneEntity.setSpecSelectTitle(mStoneEntity.getSpecSelectTitle());
            hisStoneEntity.add(stoneEntity);
            mStoneEntity.setPrice(null);
            mStoneEntity.setSpecId(null);
            mStoneEntity.setNumber(null);
            mStoneEntity.setColorId(null);
            mStoneEntity.setTypeId(null);
            mStoneEntity.setPurityId(null);
            mStoneEntity.setShapeId(null);
            mStoneEntity.setSpecTitle(null);
            L.e("克隆数据"+stoneEntity.toString());
        }
        ...
        /*当前选择后验证数量是否有输入*/
        int mpostsion;
        public void setTag(ViewHolder  viewHolder,int postsion) {
                    L.e(" viewHolder.idStoreNumber"+  viewHolder.idStoreNumber.getText());
            this.mpostsion = postsion;
            StoneEntity stoneEntity = stoneEntities.get(postsion);
            stoneEntity.setNumber(viewHolder.idStoreNumber.getText().toString());
    ...
    private void initDataAndListener(final ViewHolder viewHolder, final StoneEntity stoneEntity,final int position) {
        L.e(stoneEntity.toString());
        if (!StringUtils.isEmpty(stoneEntity.getTypeId())){
            viewHolder.idStoreType.setTextName(stoneEntity.getTypeTitle());
        }else {
            viewHolder.idStoreType.setDefaultText("类型");
        }
        if (!StringUtils.isEmpty(stoneEntity.getSpecId())){
            viewHolder.idStoreNorm.setTextName(stoneEntity.getSpecTitle());
        }else {
            viewHolder.idStoreNorm.setDefaultText("规格");
        }

        if (!StringUtils.isEmpty(stoneEntity.getShapeId())){
            viewHolder.idStoreShape.setTextName(stoneEntity.getShapeTitle());
        }else {
            viewHolder.idStoreShape.setDefaultText("形状");
        }

        if (!StringUtils.isEmpty(stoneEntity.getPurityId())){
        ...
        if (!StringUtils.isEmpty(stoneEntity.getColorId())){
